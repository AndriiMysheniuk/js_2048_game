...
function updateMessages() {
    if (game.getStatus() === (0, _gameClassJsDefault.default).STATUS.lose) loseMessage.classList.remove("hidden");
    else if (game.getStatus() === (0, _gameClassJsDefault.default).STATUS.win) winMessage.classList.remove("hidden");
}
function getScore() {
 ...
 */ class Game {
    ...
    moveUp() {
        ...
        for(let cel = 0; cel < this.state[0].length; cel++){
            ...
            for(let row = 0; row < this.state.length; row++)if (this.state[row][cel] !== 0) filteredColumn.push(this.state[row][cel]);
            const newColumn = [];
            ...
            for(let row = 0; row < this.state.length; row++)if (newState[row][cel] !== newColumn[row]) {
    ...
    moveDown() {
        ...
        for(let cel = 0; cel < this.state[0].length; cel++){
            const filteredColumn = [];
            for(let row = this.state.length - 1; row >= 0; row--)if (this.state[row][cel] !== 0) filteredColumn.push(this.state[row][cel]);
            const newColumn = [];
            for(let i = 0; i < filteredColumn.length; i++)if (filteredColumn[i] === filteredColumn[i + 1]) {
                const mergedValue = filteredColumn[i] * 2;
